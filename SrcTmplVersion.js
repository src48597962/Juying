var newVersion = {
    ...
    "SrcJuyingdesc":{
        ...
        "4.9":"智能解析逻辑针对优先上次解析做了大幅度调整；tvbox源导入增加对xpath接口的支持(存在兼容问题需自行确认好坏)；管理-扩展中心升级，界面代码大优化，增加资源导入历史记录；管理-扩展增加聚影说明；自动更新检测时间调整为24小时；TVBox订阅做大幅度的升级，代码和导入进行整合，12小时文件缓存；20221015",
        "5.0":"增强TVBOX白嫖导入，添加直播模块，功能自行体验；20221018",
        "5.1":"直播模块优化，缓存文件调整；管理扩展中部份提示优化，界面微调；修复从未调用展示时删除接口报错；20221019",
        "5.2":"修复少许小bug；加强直播模块的逻辑处理，本地为空自动导入第一个订阅改为默认显示第一个订阅，本地源编辑全改为动态刷新；聚影资源码分享和订阅同步增加直播的订阅数据；20221020"
    }
};
