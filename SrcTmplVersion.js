var newVersion = {
    ...
    "SrcJuyingdesc":{
        ...
        "4.3":"观影设置改版，更加清晰美观；解析逻辑全新升级优化，聚影智能(断插辅助开时独立调用帅助手逻辑,且支持远程断插文件)、强制断插(走断插本地小程序逻辑)、强制嗅探(只走聚影自带几个url解析)；智能拦截无效视频地址升级优化；支持断插返回json对象解析结果；批量添加解析未去重bug修复；其他细节调整；20220918"
    }
};
