var newVersion = {
    ...
    "SrcJuyingdesc":{
        ...
        "8.1":"1.修复搜狗搜索；2.修复接口搜索ocr；20230827",
        "8.2":"1.我的云盘增加显示资源库；2.云盘历史或收藏快速换源；3.共享支持显示其他文件；20230912",
        "8.3":"1.修复资源码；2.修复分享；3.增加分享方式；20231015",
        "8.4":"1.修复新剪贴板分享地址同时兼容旧版导入；2.更换1路代码仓地址；3.其他优化；20231105",
        "8.5":"1.搜索热榜改为360；2.云盘体验优化；20231126",
        "8.6":"1.更新云盘；2.主页收藏历史按钮可长按互换；3.git代理更新；4.小程序图标更新；20231226",
        "8.7":"1.修复搜狗选集解析地址；2.搜索框支新alipan地址；3.避免第1个按钮长按无管理；4.修复我的云盘；20240127",
        "8.8":"1.我的云盘加上菜单；2.修复搜索热榜；3.云盘加上长按转存、删除按钮；4.增加云盘播放记录同步，支持全渠道多端同步；5.主页菜单按钮支持长按切换；6.云盘接口允许禁用启用；7.其他几个月的累积更新，记不太清了；20240506"
    },
    "SrcJu":"3.3",
    "SrcJudesc":{
        "1.0":"聚阅发布",
        ...
        "2.5":"1.批量选择调整为子菜单，增加批量删除；2.修复导入新接口未置顶问题；3.分享及搜索测试调整为当前所见或多选列表；20230826",
        "2.6":"1.增加导入手工确认模式；2.二级选集增加rule属性；3.搜索结果兼容大小写字母；4.搜索ocr识别支持数字计算;5.二级选集标题默认统一修正(样式长按可取消)；6.重写日志打印方法；20230912",
        "2.7":"1.主页支持x5组件；2.主页快速菜单长按支持删除当前；3.二级数据返回增加novel属性；4.链接加上type避免同名不同类型覆盖问题；5.二级多处代码调整（name为空处理、切换站源name修、修复切源精准匹配）；6.主页新搜索页改为新窗口；7.支持8.10新版本动态加载样式；20231008",
        "2.8":"1.云口令导入支持二次确认页展示；20231023",
        "2.9":"1.修复新版变更分享链接造成的bug；2.二级加载失败时取消自动切源；20231031",
        "3.0":"1.分享口令兼容旧版软件导入；2.更换1路代码仓地址；3.其他优化；20231105",
        "3.1":"1.修复云口令导入接口确认页的错误；2.二级增加分页设置 ；3.二级代码逻辑优化；4.修复搜索页翻页导致的显示bug；20231110",
        "3.2":"1.长按分享增加单接口指定分享剪贴板；2.扩展完善接口信息到临时变量中；3.内置一个解密方法；4.支持自定义$.exports方便调用；20231207",
        "3.3":"1.搜索除二级放开所有返回；2.视界新版主页选择源支持图标；3.简化主页操作；20240220",
        "3.4":"1.增加支持视界聚合竖屏分栏搜索样式；2.增加支持预处理代码；3.预加载代码优化；4.二级增加自定义中间扩展组件返回属性；"
    }
};
