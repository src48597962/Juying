var newVersion = {
    ...
    "SrcJudesc":{
        ...
        "2.3":"1.修复首次进软件收藏或书架，某些源的报错；",
        "2.4":"1.修复首次使用聚阅报错及无法使用云口令问题；2.接口管理增加文件分享和导入、类型长按全选；3.接口增加更新时间显示；4.全局变量梳理，换主页源时清除接口定义变量；5.修复二级oldextra和二级url取值错误；6.增加云口令文件分享支持打开文件直接导入（需海阔8.5版）"
    }
};
