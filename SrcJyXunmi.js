        ...
        if(lists.length>0){
            ...
            lists.forEach(item => {
                ...
                if(!/qq|mgtv|iptv|iqiyi|youku|bilibili|souhu|cctv/.test(item.url)){//&&vodname.indexOf(name)>-1
                    let push = {//ishkss?vodname:(vodname!=name?vodname.replace(name,'<font color=red>'+name+'</font>'):'<font color=red>'+vodname+'</font>') + ' - <font color=#f13b66a>'+ item.website+'</font>' + (item.tags.length>0?'  ['+item.tags.join(' ')+']':''),
                        title: ishkss?vodname:(vodname!=name?vodname.replace(name,'““'+name+'””'):'““'+vodname+'””') + ' - ““'+ item.website+'””' + (item.tags.length>0?'  ['+item.tags.join(' ')+']':''),
